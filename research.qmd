...
These projects trace one question: how can wireless networks keep information **fresh and useful** for the tasks that depend on it, under real resource constraints? Each project below opens with a short summary; the full abstract is one click away.
...
Safety-critical vehicle applications need *processed* sensing information while it is still fresh — yet most Age-of-Information frameworks ignore computation delays entirely. This work proposes a computation-aware NOMA-ISCC framework that jointly optimizes task scheduling, beamforming, and power allocation, combining a hybrid multicast–unicast transmission design with a Transformer-based reinforcement learning controller. It substantially lowers the expected weighted-sum AoI over state-of-the-art benchmarks while improving spectral efficiency, latency reliability, and scalability.
...
A single roadside unit mostly observes a vehicle's radial motion, which fundamentally limits how well it can predict where to point the beam next. This work lets multiple RSUs sense cooperatively — combining monostatic and bistatic observations from standard OFDM waveforms, with no extra signaling — and proposes MR-CLNet, an end-to-end spatio-temporal network that maps multi-RSU observations directly to future beam directions. It consistently outperforms single-RSU and learning-based baselines, with the largest gains at high speeds and on curved roads.
...
Multicast group size hides a trade-off in multi-cell NOMA: larger groups suffer unstable information freshness but less inter-cell interference, while smaller groups face the reverse. This work proposes an AoI-aware interference mitigation (AIM) scheme that dynamically adjusts group size, scheduling, and power allocation — with proved optimality of the scheduling strategy — and shows that a bandit-based grouping approach outperforms random and fixed groupings. To our knowledge, this is the first study of AoI for task-oriented applications in multi-cell multicast NOMA networks.
...
Keeping IoT information fresh through LEO satellites means fighting propagation delay, satellite handovers, and inter-satellite hops — under a power budget. This work models uplink and downlink AoI end-to-end and proposes EA-AoI, a deep-reinforcement-learning scheduler with low-complexity power allocation. It achieves lower AoI than SIC-based, greedy, and random baselines on both simulated and real satellite trajectories, while keeping long-term average power within the required threshold.
